document.addEventListener("DOMContentLoaded", () => {
    ...
    //CHATGPT CODE
    function generateShareableCode(tileMap) {
        try {
            const serializedMap = JSON.stringify([...tileMap]);
            return btoa(serializedMap); // Encode to Base64
        } catch (error) {
    ...
    function loadGameFromCode(code) {
        try {
            const decodedMap = decodeGameCode(code);
            const parsedMap = parseTileMap(decodedMap);
    
            clearGameBoard();
            rebuildGameBoard(parsedMap);
            console.log("Game loaded successfully from code.");
        } catch (error) {
            console.error("Error loading game from code:", error);
            alert("Invalid game code! Please try again.");
        }
    }
    
    function decodeGameCode(code) {
        return atob(code); // Decode Base64 string
    }
    
    function parseTileMap(decodedMap) {
        return new Map(JSON.parse(decodedMap)); // Convert JSON string to Map
    }

    function rebuildGameBoard(tileMapData) {
        tileMap.clear(); // Clear the existing tile map
    
        // Rebuild each tile from the map
        for (const [coords, letter] of tileMapData) {
            const [x, y, z] = JSON.parse(coords);
            tileMap.set(coords, letter);
    
            const tile = createTile(coords, letter, x, y, z);
            addTileToLayer(tile, z);
        }
    }
    
    function createTile(coords, letter, x, y, z) {
        const tile = document.createElement("div");
        tile.className = "tile";
        tile.dataset.letter = letter;
        tile.dataset.selected = false;
        tile.dataset.coords = coords;
        tile.style = `grid-row-start: ${x + 1}; grid-column-start: ${y + 1};`;
        tile.addEventListener("click", tileClickHandler);
    
        // Add corner letters
        ["top-left", "top-right", "bottom-left", "bottom-right"].forEach(quadrant => {
            const corner = document.createElement("div");
            corner.className = `corner ${quadrant}`;
            corner.textContent = letter;
            tile.appendChild(corner);
    ...
    function addTileToLayer(tile, z) {
        if (!board[z]) {
            const layer = document.createElement("div");
            layer.className = `layer layer-${z}`;
            board[z] = layer;
            container.prepend(layer); // Add layer to the container
        }
        board[z].appendChild(tile); // Add tile to the appropriate layer
    }
    
    function clearGameBoard() {
        container.innerHTML = ""; // Clear the DOM
        board.length = 0; // Clear the board layers array
    }
    
    document.getElementById("generate-code").addEventListener("click", () => {
        const shareCode = generateShareableCode(tileMap);
        if (shareCode) {
